...
vault write -format=json pki_int/intermediate/generate/internal common_name="Consul Intermediate Authority" alt_names="localhost,127.0.0.1" ip_sans="localhost,127.0.0.1" | jq -r '.data.csr' | tee pki_intermediate.csr && \
...
vault write pki_int/roles/nomad allowed_domains="nomad" allow_subdomains=true max_ttl="720h" && \
