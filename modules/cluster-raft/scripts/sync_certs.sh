...
cat jq -r .data.issuing_ca | sudo tee /etc/consul.d/ca && \
cat jq -r .data.certificate | sudo tee /etc/consul.d/cert && \
cat jq -r .data.private_key | sudo tee /etc/consul.d/keyfile && \
