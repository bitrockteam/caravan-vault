...
locals {
  ...
  ssh_command_bastion = var.ssh_bastion_host != null ? "ssh-keyscan -H ${var.ssh_bastion_host} >> ~/.ssh/known_hosts && ssh-add ${path.module}/.ssh-key && ${local.ssh_command} -J ${var.ssh_bastion_user}@${var.ssh_bastion_host}:${var.ssh_bastion_port} -A" : ""
}
